module.exports = {
  ...
  async execute(sock, msg, args) {
    ...
    if (!mentioned.length) return sock.sendMessage(msg.key.remoteJid, { text: '❌ Mention someone to promote.' }, { quoted: msg });
    for (const user of mentioned) {
      try {
        await sock.groupParticipantsUpdate(msg.key.remoteJid, [user], 'promote');
        await sock.sendMessage(msg.key.remoteJid, { text: `⭐ Promoted @${user.split('@')[0]}`, mentions: [user] });
      } catch {
        await sock.sendMessage(msg.key.remoteJid, { text: `❌ Failed to promote @${user.split('@')[0]}.` });
      }
    }
  }
};