module.exports = {
  ...
  async execute(sock, msg, args) {
    ...
    if (!mentioned.length) return sock.sendMessage(msg.key.remoteJid, { text: '❌ Mention an admin to demote.' }, { quoted: msg });
    for (const user of mentioned) {
      try {
        await sock.groupParticipantsUpdate(msg.key.remoteJid, [user], 'demote');
        await sock.sendMessage(msg.key.remoteJid, { text: `⬇️ Demoted @${user.split('@')[0]}`, mentions: [user] });
      } catch {
        await sock.sendMessage(msg.key.remoteJid, { text: `❌ Failed to demote @${user.split('@')[0]}.` });
      }
    }
  }
};